...
{
    void defer_close(int *f)
    {
        close(*f);
    }
    void defer_free(int **p)
    {
        free(*p);
    }
...
skip_date:
            ...
            case State_Not_Started:
            {
                ...
                mvwaddnwstr(window, draw_y, draw_x, config.enable_unicode ? L" \uF62F  " : L" O  ", 4);
            ...
            case State_Doing:
            {
                ...
                mvwaddnwstr(window, draw_y, draw_x, config.enable_unicode ? L" \uF90B  " : L" @  ", 4);
            ...
            case State_Discarded:
            {
                ...
                mvwaddnwstr(window, draw_y, draw_x, config.enable_unicode ? L" \uFB81  " : L" X  ", 4);
    ...
    struct Todo_List *sorted_merge(struct Todo_List *a, struct Todo_List *b)
    {
        _Bool compare(struct Todo_List *first, struct Todo_List *secnd)
        {
            ...
            if (first->state == State_Done)
                return true;
            else if (secnd->state == State_Done)
                return false;

            if (first->state == State_Discarded)
                return true;
            else if (secnd->state == State_Discarded)
                return false;
            return true;
        } /* compare */
